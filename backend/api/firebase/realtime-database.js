...
app.get(apiUrl + "/:province/:year",  (req, res) => {
    ...
    const phoneRef = db.ref(`phone-line-stats/${province}/${year}`);
...
app.get(apiUrl ,  (req, res) => {
    ...
    let phoneRef = db.ref(`phone-line-stats`);
    console.log(req.query.year);
    if (Object.keys(req.query).length>0){
        console.log("Provincia: "+ req.query.province);
        console.log("Year: "+ req.query.year);
        console.log("landline: "+ req.query.landline_over);
        console.log("post_payment_phone_line: "+ req.query.post_payment_phone_line_over);
        console.log("wide_landline: "+ req.query.wide_landline_over);
        if (req.query.province)  phoneRef = phoneRef.orderByChild("province").equalTo(req.query.province);
        if (req.query.year) phoneRef = phoneRef.child(req.query.year);
        if (req.query.landline_over) phoneRef = phoneRef.orderByChild("landline").startAt(parseInt(req.query.landline_over));
        if (req.query.post_payment_phone_line_over) phoneRef = phoneRef.orderByChild("post_payment_phone_line").startAt(parseInt(req.query.post_payment_phone_line_over));
        if (req.query.wide_landline_over) phoneRef = phoneRef.orderByChild("wide_landline").startAt(parseInt(req.query.wide_landline_over));
    }
